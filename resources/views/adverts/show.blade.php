...
@section('content')
  ...
  <div class="row">
    <div class="col-md-9">
      ...
      <div class="row">
        <div class="col-md-12">
          @foreach ($advert->replies as $reply)
            @include ('adverts.reply')
          @endforeach
        </div>
      </div>
      @if (auth()->check())
        <div class="row">
          <div class="col-md-12">
            <form method="POST" action="{{ $advert->path() . '/replies' }}">
              {{ csrf_field() }}

              <div class="form-group">
                <textarea name="body" id="body" class="form-control" placeholder="Написать отзыв" rows="5"></textarea>
              </div>
